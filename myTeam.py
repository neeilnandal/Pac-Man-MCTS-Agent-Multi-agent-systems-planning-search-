...
def createTeam(firstIndex, secondIndex, isRed, first="PacmanAgent", second="GhostAgent"):
    ...
    The original version used eval(). V2 uses an explicit class registry so only
    known agent classes can be instantiated.
    """
    agent_registry = {
...
class PacmanAgent(CaptureAgent):
    ...
    def chooseAction(self, gameState):
        ...
        if len(available_food) < 2 or carried_food > self.FOOD_RETURN_THRESHOLD or nearby_ghosts:
